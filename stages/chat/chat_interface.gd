...
# --- CONFIGURATION ---
const RASA_URL: String = "http://localhost:5005/webhooks/rest/webhook"
...
# --- ASSETS ---
# In Godot 4, you can export a Dictionary of textures for easier drag-and-drop in the inspector
@export var guard_sprites: Dictionary = {
	"eugenia": preload("res://stages/chat/sprites/target_eugenia.png"), # Add your sprite
...
func _ready() -> void:
	# Connect Signals (Callable syntax)
...
func configure_bot(guard_name: String) -> void:
	...
	# 2. Context Switch (Guard vs Date)
	...
	# 3. Inform Rasa (Send invisible payload)
	# We use a JSON formatted string inside the text for the /inform intent
...
func _setup_date_mode() -> void:
	...
	# Reset bars for a new encounter
...
func _on_send_pressed(text_submitted: String = "") -> void:
	...
	user_input.text = ""
	# Append to a chat log if you have one
	_send_to_rasa(text)
...
func _send_to_rasa(message_text: String) -> void:
	...
	http_request.request(RASA_URL, headers, HTTPClient.METHOD_POST, body)

# --- RECEIVING DATA ---
# Godot 4.x Signature: result is int, headers is PackedStringArray, body is PackedByteArray
...
func _process_single_message(message: Dictionary) -> void:
	...
	if "custom" in message:
		...
		if "ai_score_modifier" in data:
			_handle_guard_visuals(data["ai_score_modifier"])
...
func _add_to_log(sender: String, text: String) -> void:
	...
	# Append text with timestamp (Optional) or just formatting
